...
public class DefaultExtractionController implements ExtractionController {
    ...
    protected List<Map<String, Object>> getQueriesResult(ReportLoaderFactory loaderFactory, ExtractionContext context) {
        return getQueriesResult(getQueries(context).iterator(), loaderFactory, context);
    }

    protected List<Map<String, Object>> getQueriesResult(Iterator<ReportQuery> queryIterator, ReportLoaderFactory loaderFactory, ExtractionContext context) {
        ReportQuery firstReportQuery = queryIterator.next();

        //gets data from first dataset
        List<Map<String, Object>> result = getQueryData(loaderFactory, context, firstReportQuery);

        //adds data from second and following datasets to result
        while (queryIterator.hasNext()) {
            ReportQuery reportQuery = queryIterator.next();
            List<Map<String, Object>> currentQueryData = getQueryData(loaderFactory, context, reportQuery);
